...
| 4 | **Partnership not accepted** | `submitPayments` / `submitSinglePayment` (`features/accounts-hub/actions.ts`), partnership gate | **Collab-wide block.** Every payment write (draft or UTR-bearing) requires the creator's real `partnership_status='approved'` on the post and every sibling sharing the `collab_id`. Ads Usage Rights and `ad_partnership_valid`/Partnership Key admin overrides do not bypass this payment rule. Rejections are returned in `blockedByAdPartnership`, with `blockedDetails[].partnershipMissingSiblings` naming the offending `post_id_short`(s). |
...
When a collab's posting is submitted, `submitPosting` (`features/posting/actions.ts`) calls **`autoInitDraftPayment`**, which delegates to the transactional `reconcile_creator_payment_eligibility` RPC. Posting itself does not set a payment status. The RPC:

1. Takes a per-collab advisory lock and resolves the representative (lowest `post_id`).
2. Is **idempotent** and DB-guarded by a partial unique index allowing at most one NULL-UTR draft per representative.
3. Enforces **collab-level eligibility** — no draft until *every* deliverable has both `post_link` and `post_date` and the creator accepted the partnership (`partnership_status='approved'`). Admin keys never bypass acceptance.
4. Inserts a `Not Due` row with `amount` = full collab total, `due_date = post_date + 30 days`, and the next 15th/30th estimated cycle, then mirrors `Not Due` across the collab's post rows.
5. Runs after every partnership sync. Approval may create the draft; pending/rejected/revoked removes only empty Not Due/Due drafts and mirrors. UTR-bearing installments plus Partial/Done history are never removed.
...
| **Partnership acceptance** | The creator has not accepted the partnership (`partnership_status != 'approved'`) on the row or any sibling. Applies with or without UTR and regardless of Ads Usage Rights | `blockedByAdPartnership` |
| **Dedup / already-paid** | Collab already fully paid (`paidSoFar ≥ collabTotal`), or exact `(post_id, lower(utr))` re-submission | `duplicates` |

The partnership gate fires on **every payment write**, including UTR-less drafts. A creator's accepted state is mandatory for all collabs; Ads Usage Rights and admin Partnership Keys are irrelevant to this payment decision. Each blocked row is returned with a `BlockedDetail { postId, unpostedSiblings, partnershipMissingSiblings }` so the toast can name the exact siblings to fix.

Accepted rows are handed to `record_eligible_collab_payment`, which rechecks eligibility inside the database transaction, serializes writes with a per-collab advisory lock, maps child Post IDs to the representative, and recomputes paid-so-far before inserting. `payments_enforce_installment_total` uses the same lock at the ledger boundary and rejects any UTR-bearing write that exceeds the remaining collab balance, including direct writes outside the app. Installments (UTR present) are immutable rows stamped with the collab rollup (`Partial` or `Done`); the NULL-UTR draft is updated in place; `posts.payment_status` / `utr` / `payment_date` are cascaded to every sibling without deleting prior financial history. The result envelope is `{ ok, saved, paid, due, partial, skipped, skippedIds, blockedByStage, blockedByReelRule, blockedByAdPartnership, duplicates, blockedDetails }`. After writing, the Meta Ads warehouse covered set (5s timeout fallback) stamps `posted_but_not_tested` on each row — annotation only, never blocking. Cache is invalidated via `revalidateTag("payments" | "posts")` and `revalidatePath` for `/accounts-hub`, `/journey`, `/my-dashboard`.
