...
- **Buckets:** Untested = not classified AND not in warehouse; Ad-Run = classified (`ads_results` non-empty) OR in warehouse. Historic rows: Ad Run only. Ad Run sorted primary-ad spend desc, unmatched → post_date desc.
- **KPI:** totalEligible, classified, inMetaAds, pendingClassification, winners, discarded (all legacy, live-only, semantics unchanged) + `kpi.categories` {incrementalWinners, winners, p0, p1, p2, discarded} — one tick per matched row (live+historic) at its best category. Strip = Eligible/Untested/In Meta Ads + 6 category HeroKpi tiles.
