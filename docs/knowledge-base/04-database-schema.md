...
| `onboarded_by`                                                                          | text        | yes  | —            | actor who completed onboarding. Reach-out rows leave this NULL; posting's "Onboarded by" filter uses this (rows there are already onboarded). |
...
> **Owner repair (2026-07-22):** A later post-ID migration accidentally wrote new Reach Out owners into `onboarded_by`. Migration `20260722123000_repair_reachout_owner_attribution` moved 1,078 affected live Reach Out owners into `logged_by`, cleared the invalid onboarding value, and corrected `submit_reachout`. Rows with neither owner remain intentionally unattributed.
...
| `content_reminder_sent_at` / `posting_pending_sent_at` / `onboarding_pending_sent_at` / `delivery_reminder_sent_at` | timestamptz | yes  | —            | Cron idempotency flags (delivery_reminder = creator nudge 2 days before est_delivery, 2026-07-21)                                                                                                                                                                            |
...
| `is_blacklisted`                                                                        | boolean            | NOT NULL default false; creator-level terminal offboarding flag                                                                                                                                                                                                               |
| `blacklist_reason`                                                                      | text               | mandatory when blacklisted                                                                                                                                                                                                                                                    |
| `blacklisted_at` / `blacklisted_by`                                                     | timestamptz / text | offboarding timestamp and actor email                                                                                                                                                                                                                                         |
| `blacklist_evidence`                                                                    | jsonb              | overdue post/collab/campaign/team/deadline snapshot captured at decision time                                                                                                                                                                                                 |
| `created_at` / `updated_at`                                                             | timestamptz        |                                                                                                                                                                                                                                                                               |

**Constraints/Indexes:** `inf_id` UNIQUE, `sif_number` UNIQUE, `profile_id` UNIQUE, `username` UNIQUE, `category` index, partial `blacklisted_at DESC` index. `creators_blacklist_fields_check` requires nonblank reason + actor and a timestamp whenever `is_blacklisted=true`.

### Table: `creator_audit_log`

Append-only history for creator offboarding. `creators_blacklist_audit` writes the row in the same transaction as a false→true blacklist change. Fields: `creator_inf_id`, `username`, `action='offboarded'`, mandatory `reason`, `actor_email`, `metadata` JSONB, `created_at`. RLS is enabled; anon/authenticated have no table grants; the application service role has `SELECT` and `INSERT` only.
...
| `user_prefs`              | Per-user UI preference store `(email, key) → jsonb` (2026-07-21)      | First key `reachout_pins` = outbound reach-out sticky pre-selections `{campaignId, gender, contentType}` (presence = pin ON); RLS enabled, **no policy** (service-only)                                                                                                |
...
- **`posts.workflow_status`** (`posts_workflow_status_check`): `Reach Out`, `On Board`, `Posted`, `Delivered`, `RTO`, `Cancelled`, `Cancelled After RTO`, `Offboarding`/`Offboarded`, plus legacy values. _(The 2026-06-07 work added `Offboarded`; `types.gen.ts` also lists aspirational values like `Order Sent`/`Awaiting Reply`/`Declined` — verify against the live CHECK before relying on them.)_
...
| `purge_test_rows`                                                  | `p_source_table text, p_scope text, p_deleted_by text`                                                                                                                                                                                                                 | `integer`                                                               | **Test Mode purge** (SECURITY DEFINER, granted service_role). Allowlist `campaigns/posts/creators/payments`. Archives `is_test` rows of the table into `test_mode_archive` (`row_data = to_jsonb(row)`, scope + deleted_by), then `delete where is_test`, returns deleted count. Caller (`setTestMode`) loops in FK-safe order payment→collab→creator→campaign. (`testmode_p2_purge_rpc`)                                                                                                                                                                                        |

### Test Mode / config tables (2026-06-24, `testmode_p1_foundation` + `testmode_p2_purge_rpc`)

- **`app_settings`** `(key text pk, value text, updated_at timestamptz, updated_by text)` — key/value config store. Keys: `test_mode_scopes` (JSON array string, `[]`=off), `campaign_auto_close_enabled` (`'true'`/`'false'`, default-ON when absent).
- **`is_test boolean not null default false`** added to `campaigns`, `posts`, `creators`, `payments` (+ partial index `where is_test`). Stamped on create when the matching Test Mode scope is on; purged on scope-off.
- **`test_mode_archive`** `(id bigint pk, source_table text, scope text, row_pk text, row_data jsonb, deleted_by text, deleted_at timestamptz default now(), restored boolean default false)` — every purged test row archived here verbatim for recovery.
...
> **Update 2026-06-25c (post_id minted at onboarding)**: `posts.post_id` is now NULLABLE (reach-out rows carry NULL post_id/post_number/collab; identified by bigserial `id` = the real PK; plain UNIQUE allows multiple NULLs). `submit_reachout`/`create_repeat_collab` RETURN the bigserial `id` and insert NULL ids. New `mint_onboarding_block(p_inf_id, p_order_id, p_deliverable_count)` RETURNS (collab_number, collab_id, start_post_number, post_id_base) — advisory-locked; reuses the collab on the normalized order_id else next C; maxP/maxC over `posts` ∪ `historic_posts`; union columns table-aliased to avoid OUT-param ambiguity. `mint_collab_for_order` superseded (kept, unused).

> **Master ID & Nomenclature Guide** — the canonical reference for every identifier rule (SIF / P / C / nomenclature / campaign, new + historic) lives at `docs/Master-ID-Nomenclature-Guide.html` (+ `.pdf`). Keep it in sync with this chapter and [[project_collab_deliverable_numbering_rule]] on any ID-logic change.
