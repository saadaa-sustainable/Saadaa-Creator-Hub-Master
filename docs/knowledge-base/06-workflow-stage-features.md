...
- **Gender is manual-only (2026-07-20):** Meta/Instagram never return gender, so gender is NEVER auto-filled and NEVER defaulted to a guess. Outbound form: the lookup no longer sets `gender`; the select is no longer `disabled`/AUTO-locked for existing creators (was locking a stale/wrong stored value — e.g. Female on a male creator); `REACHOUT_DEFAULTS.gender = ""` so it starts blank each fetch and `z.enum(GENDERS)` still enforces it on submit. Inbound: per-row lookup keeps `gender: row.gender` (never overrides the CSV/manual value); CSV import + `newRow` default to `""` (not `"Female"`) so blank rows are flagged incomplete via `isRowValid`/`getRowValidationErrors` until the user picks. The stored `creators.gender` is still written from the manually-chosen value. 
- **Sticky pins (2026-07-21):** Campaign / Gender / Content Type carry per-user tick toggles (`PinTick` in `outbound-form.tsx`). Pins live in `user_prefs` key `reachout_pins` via `features/reach-out/prefs-actions.ts` (`getReachoutPins` server-read in page.tsx — pinned campaign validated against the live select list; `setReachoutPin` fire-and-forget writes keyed on the signed-in email). Presence = ON; ON keeps the value across submit-reset + next visit; changing a pinned field re-points the pin; OFF → normal blank reset. Pinned gender is re-applied after the lookup's AUTO_FIELDS wipe (it's the user's manual choice — gender itself stays never-auto-filled). All pins default OFF. `?campaign=` URL param still beats the campaign pin.
...
**Overdue KPI + instant search (2026-07-16, `2bb1db4`):** both Onboarding and Posting KPI strips gained a funnel-parity Overdue tile (parent rows, >15d since reach, no post). Free-text search on both pages is client-side (`lib/live-search.ts`) — the `q` URL param only seeds/mirrors, never triggers a server render.
